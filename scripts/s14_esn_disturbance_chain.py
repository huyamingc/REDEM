...
def main():
    ...
    payload = _nan_to_null({
        ...
        'null_reason': ('non-finite aggregate values are written as null '
                        '(undefined, e.g. a readout-timing cell in which no '
                        'run yielded a finite qualification score, so the '
                        'kappa mean / std and the drift contrast have no '
                        'measurement)')})
...
def _nan_to_null(obj):
    ...
    undefined aggregates (kappa mean / std and the drift contrast of a readout
    timing cell in which no run yielded a finite qualification score), which is
    not valid JSON and is rejected by strict parsers.
    """
