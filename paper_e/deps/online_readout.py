...
class ThreeFactorReadout:
    ...
    def __init__(self, n_features, mode='reward', learning_rate=0.05,
                 elig_decay=0.9, seed=0, w_max=None):
        self.n_features = int(n_features)
        self.mode = mode
        ...
        # Optional soft L2 norm cap on the weight vector (bias-runaway fix,
        # s45): w *= W_MAX/||w|| whenever an update pushes past the cap.
        # None (default) = unbounded, preserving pre-s45 behavior exactly.
    ...
    def consolidate(self, third_factor, reset=True):
        ...
        If w_max was set at construction, a soft L2 norm cap is applied to w
        after the update (w *= w_max/||w|| when ||w|| > w_max) -- the
        bias-runaway / anti-learning fix from s45, now available at the
        readout level instead of in each experiment script.
        """
        self.w += self.learning_rate * float(third_factor) * self.e
        if self.w_max is not None:
            nw = float(np.linalg.norm(self.w))
            if nw > self.w_max:
                self.w *= (self.w_max / nw)
        if reset:
            self.e[:] = 0.0
