...
% Target: International Journal of Bifurcation and Chaos (World Scientific)
%         or Chaos: An Interdisciplinary Journal of Nonlinear Science (AIP)
%
% COMPILATION: latexmk -pdf PAPER_A.tex   (or pdflatex x2)
%
% SUBMISSION NOTES:
%  - For IJBC: replace the document class with the journal's ws-ijbc.cls
%    (World Scientific template). For Chaos: use revtex4-2 (AIP template).
...
order--chaos transition at $\kappa^\ast \in (25,30)$, with the held-out memory
capacity peaking 24--53\% above the uncoupled baseline just before the
transition, and deep chaos destroying memory. The decay of linear memory
follows an analytically derived forgetting kernel $M(t)=\int p(\tau)e^{-t/\tau}
\,d\tau$ over the log-normal trap spectrum (Pearson $r=0.97$ against the
measured memory-capacity curve): the $1/e$ horizon is pinned near
$\tau_0/\bar{\Delta t}\approx 16$ pulses regardless of spectrum width, while the
width CV controls the tail weight. Finally, a homeostatic regulator that
...
foundations are well established: echo state networks \cite{jaeger2001} and
...
spread of activation energies (Appendix~A.1).
...
ratios (row-normalized weights). The physical clip range
...
order-independent (Appendix~A.2). At $\kappa=0$ the model reduces exactly to
...
window is $N_{\mathrm{eff}} = \tau_0/\bar{\Dt} \approx 17$ pulses.
...
the full stream (detailed iteration steps in Appendix~A.4).
...
memory in the chaotic regime by 2--3$\times$ (Section~\ref{sec:chaos}). The
estimator derivation and the linear-reservoir closed form that anchors the
theory are given in Appendix~A.5.
...
\midrule
ring\_bidir      & negative & order--chaos at $\kappa^\ast\in(25,30)$ & $+30\%$ at $\kappa=20$ \\
lateral\_ring    & negative & order--chaos at $\kappa^\ast\in(25,30)$ & $+24\%$ at $\kappa=20$ \\
random\_graph    & negative & order--chaos at $\kappa^\ast\in(25,30)$ & 13.91 ($+53\%$) at $\kappa=25$ \\
ring\_unidir     & positive & self-limited criticality ($\lambda\approx-0.002$), $\kappa\in[1,10]$ & 0.12--0.39$^{\ast}$ \\
...
$\approx -0.002$ (within our FTLE resolution of zero) over a full decade of
coupling strength $\kappa\in[1,10]$, held there by the $\alpha_{\mathrm{eff}}$
clip (clip fraction 0.5--1.0) interacting with the $(1-x)$ saturation. This
...
(a 3--7$\times$ drop from the peak). The in-sample (train-segment) MC, by
...
precision; derivation in Appendix~A.3). Three properties follow
(Fig.~\ref{fig:forgetting}):
\begin{enumerate}
\item \textbf{The $1/e$ horizon is median-pinned.} $M(\tau_0/\bar{\Dt})
\approx e^{-1}$ regardless of CV; the horizon is 12--16 pulses for CV
$\in[0.02,1.0]$, in agreement with the nominal window $N_{\mathrm{eff}}\approx
17$. The \emph{median} time constant sets where memory is; the width sets how
it decays.
...
$M(k\cdot\bar{\Dt})$ at CV $=0.20$ with \textbf{Pearson $r=0.97$}; at lag 10
...
CV $\in[0.02,1.0]$ is in Fig.~\ref{fig:forgetting} and Appendix~A.3.
...
homogenizing edges), and temperature drift moves the optimum to larger
$\kappa$. Consequently, any regulator anchored to a \emph{fixed activity
...
400-pulse Benettin pair at the current $\kappa$ (20\% computational overhead)
and steps
\begin{equation}
\kappa \leftarrow \mathrm{clip}\big(\kappa + \eta\,\mathrm{clip}
(\lambda_{\mathrm{target}} - \hat\lambda,\, -1, 1),\, [1,60]\big),
\qquad \lambda_{\mathrm{target}} = -0.02,
\label{eq:homeostat}
\end{equation}
...
(noise) over the fixed coupling (Table~\ref{tab:homeostat}), at task-level NMSE
parity (the RLS readout absorbs task-level differences; the substrate-level
memory improvement is revealed by the readout-independent MC probe). The
...
fixes both the $1/e$ memory horizon and the forgetting tail). For the
neuromorphic-design question ``what can this device compute'', the answer is: a
bounded but substantial region of the memory--chaos plane, fully characterized
here.
...
trace, and the separation must be exploited nonlinearly (a companion work uses
these states for structure-plasticity signals). For linear readouts, the
...
$\lambda \approx -0.05$), exactly where the homeostat parks the system.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{paperA_fig1_substrate.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{substrate_phase_diagram_v2.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{forgetting_curve_theory.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\textwidth]{paperA_fig4_robustness.png}
...
\cite{prior} (verified to $\le 10^{-15}$ in the implementation's self-test).
...
16$--17 pulses regardless of CV (numerically 12--16 pulses for CV
...
$\ln M(t)\approx-(\ln\tau^*-\mu)^2/(2\sigma^2)-t/\tau^*\approx-(\ln t-\mu)^2/
(2\sigma^2)$ to leading order. The log--log slope therefore asymptotes to
\begin{equation}
\frac{d\ln M}{d\ln t}\approx -\frac{\ln t-\mu}{\sigma^2}.
\label{eq:tailslope}
\end{equation}
At $t=200$ pulses, CV $=0.2$ ($\sigma^2=0.0392$;
$\mu=\ln(\tau_0/\bar{\Dt})\approx2.76$ in pulse units), this predicts $-64.8$,
in agreement with the measured long-lag slope $-60.6$ (the residual difference
is the sub-leading $t/\tau^*$ term). Narrowing the spectrum (CV$\to0$) makes
$\sigma^2\to0$ and the slope $\to-\infty$: the single-exponential limit.
...
component $A^{k-1}b\,u_{t-k}$, whose covariance with $x_t$ is
$c_k=A^{k-1}b\,\mathrm{Var}(u)$. The best linear predictor achieves the
...
\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{paperA_figS1_cv_sweep.png}
...
\bibitem{prior} Author's prior manuscript: Si$_3$N$_4$ pulse-encoding
(substrate calibration source; target: \emph{Neuromorphic Computing and
Engineering}).
...
unified learning with meta-adaptation and structural plasticity (target:
