...
outputs by perplexity, not bounds on recoverable information. Within
...
do not repair the state readout: stream perplexity 77.19 / 72.21
(oracle 37.1 / 38.5; the sharper gate is worse, so the negative is not a
tuning artifact), 0/10. Selectivity in real Mamba-style hosts is
...
pooled-table ceiling 7.34 (dotted).}
\label{fig:p1arms}
\end{figure}
