...
within 225--616 pulses, while frozen batch learners (GRU, tiny
transformer) --- near-perfect before the drift --- become systematically wrong after
...
spectrum is set by shallow-trap device physics (companion Paper~A), and
the slow metadata trace is shown to be a substrate-agnostic mechanism that
transfers its statistical-memory benefit to a matched echo-state
network; disturbance-robustness transfer is falsified in companion Paper~C. On standard benchmarks REDEM is
...
\emph{meta-adaptation}; and (iv) \emph{slow structure
...
first 30\% reach 0.86--1.00 pre-swap and invert to 0.00--0.15 post-swap. The
...
vs.\ 202--211 for the dual readout (factor 1.3--1.4 on T200, 1.79--2.40 on the
...
fixed coupling ($+7.6\%$ in the undisturbed control), at broadly
...
MC peaks near the edge of chaos at every CV --- $\lambda_{\mathrm{target}}{=}0$
...
transfer does not hold (companion Paper~C): recovery is the homeostat's
role, not the metadata's. REDEM's differentiation is the mechanism set
...
Substrate Physics are Non-Substitutable (three-mechanism
disentanglement), \emph{companion preprint} (2026),
