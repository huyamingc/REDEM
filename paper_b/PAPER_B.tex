...
\author{[Author Name]\thanks{Affiliation, email --- fill before submission.}}
...
9--20$\times$; the chaos homeostat restores 8--18\% of post-disturbance
...
spectrum is set by shallow-trap device physics (companion theory paper), and
...
competitive with a well-tuned ESN (drift accuracy 0.991 vs.\ 0.998;
Mackey--Glass NMSE 0.0018 vs.\ 0.0002); its differentiated value is the
...
cannot do and fix the design: at the readout an error channel is necessary;
reward and intrinsic signals are re-purposed to structural exploration.
...
the online weight update (training == inference). M4 and M5 are coupled: the
homeostat's $\kappa$ set-point gates the rewiring rate, and rewiring changes
the structure that feeds the Lyapunov estimate (dotted loop in
Fig.~\ref{fig:redem}).

The substrate follows the model of the companion theory paper (Paper A,
Section~2), with the $\tau$-spectrum, the coupling topologies, and the
...
rewards are structurally blind --- is what fixes the readout as error-driven
RLS and re-purposes reward/intrinsic signals to structure-level exploration
(M4).
...
model, uses 5 seeds ($\times2$ training cost, Table~\ref{tab:bench}).
...
readout is error-driven RLS; reward and intrinsic signals are re-purposed to
\emph{structural exploration} (M4), where no class direction is needed, only
novelty. First-order error-gated LMS ($\eta=10^{-4}$) does track the inversion
...
and --- the larger effect --- adapts to regime boundaries 9--20$\times$ faster
(3--12 pulses vs.\ 66--105 pulses): the slow trace's low-pass character smooths
the statistical transition. The metadata-only readout nearly matches the dual
...
plasticity protocol (3 seeds, 7 arms). All leak arms show $<0.02$~pp accuracy
change vs.\ normal ($0.9966$--$0.9965$ vs.\ $0.9965$), confirming causal
cleanliness --- no mechanism benefits from future information. The
causal-split protocol ($0.9964$) is within $0.01$~pp of normal, validating
that M4's block-level correlation can be computed on past data only.
...
adaptation 3 pulses) matches or beats every single-mechanism ablation:
removing the metadata drops to 0.988 (adaptation 36), removing plasticity to
0.994 (10), removing the homeostat ties at 0.996 (3, its value is in disturbed
...
$p<0.0001$). At
$N=1024$ (3 seeds, computational cost) the full system still beats the
baseline (0.998 vs.\ 0.976).

\textbf{Batch baselines.} On drift\_binary: REDEM 0.991 and a matched online
ESN 0.998 track the drift; the frozen GRU (0.371; pre 0.923 $\rightarrow$ post
0.070) and tiny transformer (0.351; pre 1.000 $\rightarrow$ post 0.000) invert
and never recover. On Mackey--Glass: REDEM NMSE 0.0018 and ESN 0.0002 forecast
well; the frozen GRU (1.30) and transformer (1.07) are worse than the mean
...
physical plausibility --- at competitive standard-task performance.
...
\midrule
drift\_binary accuracy & 0.991 & 0.998 & 0.371 & 0.351 \\
Mackey--Glass NMSE     & 0.0018 & 0.0002 & 1.30 & 1.07 \\
\bottomrule
\end{tabular}
\end{table}
...
$\rightarrow$ 0.998, adaptation 11 $\rightarrow$ 0 pulses) and REDEM (baseline
...
tasks (drift accuracy 0.991 vs.\ 0.998; Mackey--Glass NMSE 0.0018 vs.\ 0.0002)
...
readout an error channel is necessary; reward and intrinsic signals belong at
the structure level. Honest benchmarking positions REDEM in the online/physical
