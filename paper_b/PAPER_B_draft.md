...
On regime_switch (where fast memory is provably insufficient), the dual-timescale readout (fast + metadata, $\tau_m\in\{200,1000\}$) outperforms the fast-only readout by +1.3 pp on the uncoupled substrate (paired $t=19.6$, $p<0.0001$) and +2.1 pp on the near-critical substrate ($t=20.2$, $p<0.0001$), and — the larger effect — adapts to regime boundaries 9–20× faster (3–12 pulses vs 66–105 pulses): the slow trace's low-pass character smooths the statistical transition. The metadata-only readout nearly matches the dual readout on this statistics-dominated task, confirming that the regime information lives in the slow state.
