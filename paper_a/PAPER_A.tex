...
bracket above, with bootstrap 95\% CIs of $\pm0.1$--$0.8$ --- the
transition is sharp at unit resolution, not a grid artifact. Second,
...
$\kappa^\ast = 25.3\to25.4$ (lateral\_ring; shift $+0.05$, CI
$[-0.19,+0.29]$) and $27.9\to27.7$ (random\_graph; shift $-0.21$, CI
$[-0.48,+0.09]$). The order--chaos transition is therefore
...
crossing moves $+3.4$ (CI $[+2.1,+4.9]$) under the wider clip, so the
...
kernel-shaped. Fitting an \emph{effective} log-normal spectrum to
$\sqrt{\mathrm{MC}(k)}$ at each operating point
(data/substrate\_phase\_diagram\_v2.json; quadrature as in
Appendix~\ref{app:quadrature}) gives Pearson $r \ge 0.86$ against the
best fit at every $\kappa \in \{15,\dots,30\}$ of all three mode-1
topologies, and the \emph{physical} spectrum $(174\,\mu$s, CV $0.20)$
already achieves $r = 0.84$--$0.95$ over the same range ($r=0.95$ at
random\_graph $\kappa{=}25$). The fitted effective median stretches
monotonically with $\kappa$ (from $\approx 1\times$ to $2$--$3\times$
the physical value as the transition is approached) --- a
coupling-induced time dilation of the memory kernel consistent with the
near-critical slowdown of Section~\ref{sec:negfb}. Single curves cannot
separate the effective median from the width (the fit saturates at wide
CV), so we report the shape stability and the dilation qualitatively
rather than as a precise renormalization map.
...
5 seeds; companion paper Table~2) identifies $\lambda_{\mathrm{target}}{=}0$
(the edge of chaos) as optimal: post-disturbance MC improves monotonically as
$\lambda_{\mathrm{target}}\to 0$, with gains of $+25\%$ (CV=0.1), $+19\%$
(CV=0.2) over fixed coupling --- updating the manually tuned $-0.02$ used in
the main results.
...
(median $\tau_0$: $M(\tau_0)\approx e^{-1}$ regardless of CV, measured
...
contradiction (the effective median drifts $1$--$3\times$ under
coupling, Section~\ref{sec:kernel}).
...
Non-Stationary Environments (target: \emph{Neural Networks}).
