...
to low-power neuromorphic substrates. We present REDEM, a
training-inference-unified online learning architecture in which every learning
signal is local and derived from live inference: an error-driven
...
substrate memory; and the integrated system beats every single-mechanism
ablation (0.996 vs.\ 0.973, $p<0.0001$), persisting at $N=1024$. The substrate
...
REDEM --- in which every learning signal is local and derived from live
...
g_i(t)),0.001,0.10)$, current ratios $i_i=e^{\gamma x_i}$, random-graph
...
\lambda_{\mathrm{target}}=-0.02 \ \text{(slightly ordered)}.
\label{eq:homeostat}
\end{equation}
...
substrates across compared configs). \textbf{drift\_binary}: two-class interval
...
$W \mathrel{+}= \eta R e$ with $R=\pm1$ correctness only, no class label) and
...
RLS (dense)   & prediction error                    & 0.99--1.00 & 0.93--0.95$^\dagger$ & 0.983--0.991 \\
\bottomrule
\end{tabular}

\medskip
\footnotesize{$^*$initial learning destroyed. $^\dagger$post-swap recovery range
includes the sparse block-end-label condition.}
\end{table}
...
adaptation 3 pulses) beats every single-mechanism ablation: removing the
metadata drops to 0.988 (adaptation 36), removing plasticity to 0.994 (10),
removing the homeostat to 0.996 (3, its value is in disturbed scenarios per
Section~\ref{sec:homeo}); the full system beats the bare baseline (RLS on fast
features, fixed substrate) by +2.28~pp (paired $t=19.4$, $p<0.0001$). At
$N=1024$ the full system still beats the baseline (0.998 vs.\ 0.976).
...
steady accuracy 1.000 for all). Two conclusions follow. (i) The metadata is a
...
regulation of excitability, and slow structural plasticity of connectivity. The
...
that beats every ablation and survives scale-up to $N=1024$. The two negative
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{paperB_fig1_redem.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{s2_online_readout_v1.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{paperB_fig3_metadata.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{paperA_fig4_robustness.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{paperB_fig5_ablation.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{paperB_fig6_showdown.png}
...
relaxation substrate (substrate theory; target: \emph{International Journal of
Bifurcation and Chaos} / \emph{Chaos}).
