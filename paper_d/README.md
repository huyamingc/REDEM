...
- Target history: ~~*PRX Intelligence* (APS)~~ **desk-rejected (2026-09): scope mismatch**
  ...
  *Applied Intelligence* (Springer, `PAPER_D_AI.tex`).
- **Current submission version (Neurocomputing):**
  [`PAPER_D_NC.tex`](PAPER_D_NC.tex) /
  [`PAPER_D_NC.pdf`](PAPER_D_NC.pdf) — `elsarticle` class.
  Highlights: [`Highlights_D_NC.txt`](Highlights_D_NC.txt).
  Cover letter: [`COVER_LETTER_D_NC.txt`](COVER_LETTER_D_NC.txt).
  Change log vs AI draft: [`CHANGELOG_PAPER_D_NC.md`](CHANGELOG_PAPER_D_NC.md).
- AI intermediate files kept for rollback:
  [`PAPER_D_AI.tex`](PAPER_D_AI.tex) /
  [`PAPER_D_AI.pdf`](PAPER_D_AI.pdf) /
  [`Highlights_D_AI.txt`](Highlights_D_AI.txt) /
  [`COVER_LETTER_D_AI.txt`](COVER_LETTER_D_AI.txt).
...
Metric-sensitivity table: `PAPER_D_NC.tex` §Metric sensitivity
(`tab:sens`).
...
```powershell
pdflatex PAPER_D_NC.tex   # current version; run twice for cross-references
pdflatex PAPER_D.tex      # superseded REVTeX version
```
