...
- Papers A, B, C are submitted and D was rejected. Their committed data was
  generated with the **original** shared core.
- The 2026-09-08 extensions (w_max, the four 2-D/3-D/4-D input-encoding
  ...
  data is unaffected.
- Hygiene: the shared `scripts/` core was **reverted to its original (.bak)
  state** so A–D's canonical code is untouched, and Paper E carries its own
  frozen copies here — a clean architectural separation.
...
```powershell
$env:PYTHONUNBUFFERED=1; python ..\scripts\s58b_relative_sense_stress.py
```
...
versions here and record the change in
`../review_workspace/modification_log_scripts.md`.
