...
Alpha_eff clip rule and its equivalence to the manuscript spelling:
  The bound is applied to the injection COEFFICIENT before it multiplies
  (1 - x_i) (step 3 above, implemented in _couple_and_update_nb). The
  manuscript writes the same physics with the outer clip on the injected
  term, clip(x_i + alpha_eff_i * (1 - x_i)) * exp(-pw/tau) * exp(-dt/tau),
  i.e. with the clip applied before the two relaxation factors instead of
  after them. The two spellings are exactly equivalent here: for x in
  [0, 1] and alpha in [0, 1] the injected value already lies in [0, 1], and
  both relaxation factors are <= 1, so the outer clip never triggers. A
  50-pulse comparison of the two orderings on the deployed random graph
  (N=256, kappa=25) gives a maximum absolute state difference of 0.0
  (feature-level diff = 0.000e+00), and the recorded state range stays
  inside [0, 1] throughout. The clip that does matter is the alpha_eff
  clip of step 3.
...
Measured saturation at the deployed coupling:
  ...
  alpha_min. Every trajectory runner below returns this fraction as
  `clip_frac`, together with `g_abs_mean` (the mean |g_i|), so the
  saturation level of any run is recoverable from its output.
...
# Measured fraction of unit-pulse alpha_eff values that land on a clip boundary
# at the deployed operating point: random graph, N=256, kappa=25, alpha_max=0.10.
# Value and provenance: data/s27_clip_kappa_fine_v1.json, aggregate with
# topology='random_graph', kappa=25.0, alpha_max=0.1 (the 10-seed mean of the
# per-run `alpha_clip_frac` column in data/s27_clip_kappa_fine_v1.csv, which
# spans 0.3841-0.4006 across those seeds). Recorded here because the manuscript
# quotes the saturation level of the deployed coupling.
CLIP_FRAC_DEPLOYED = 0.39293

# The contrast thresholds at which that saturation begins follow from the same
# constants: alpha0*(1 + kappa*g) >= alpha_max gives g >= +0.16, and
# alpha0*(1 + kappa*g) <= alpha_min gives g <= -0.038, at kappa=25.
    ...
    if mode == 3:
        ...
        for i in range(n):
            ...
            if indptr[i + 1] > indptr[i]:
                ...
                # alpha_eff clip (the physically meaningful one): the bound is
                # applied to the coefficient, before it multiplies (1 - x_i).
                # The manuscript writes the same physics with the clip on the
                # injected term before the relaxation factors; the two forms
                # agree exactly, because x + alpha*(1 - x) is already in [0, 1]
                # for x, alpha in [0, 1] and both decay factors are <= 1, so the
                # outer clip never triggers. diag[0] counts the saturations
                # below, and run_trajectory_nb returns diag[0]/diag[2] as
                # clip_frac: at the deployed kappa=25 this is about 0.39 on the
                # random graph (see CLIP_FRAC_DEPLOYED), i.e. g >= +0.16 pins
                # alpha_eff at alpha_max and g <= -0.038 pins it at alpha_min.
                if a < alpha_min:
                    a = alpha_min
                    diag[0] += 1.0
                elif a > alpha_max:
