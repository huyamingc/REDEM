...
- [ ] Title deliberately contains no "physics" (S1 review decision); the
      physical substrate appears as background motivation (§1) and as the
      material basis of the multi-timescale memory (abstract, §2, Discussion).
- [x] Abstract leads with the differentiated mechanism set (S2); the honest
      ESN parity comparison sits in the final paragraph.
- [x] Negative results compressed to Table 1 + three sentences (S3); full
      data in `../data/s3_three_factor_v1.*` and
      `../data/s4_intrinsic_reward_v1.*`.
- [x] Fig. 1 shows the M4 ↔ M5 coupling loop (T2): κ gates the rewiring rate,
      rewiring feeds the Lyapunov estimate.
- [x] Metadata-transfer conclusion honest (T1): the slow-state mechanism's
      statistical-memory benefit transfers to a matched ESN; disturbance-
      robustness transfer is falsified in companion Paper C
      (`\cite{companionC}` added 2026-08-25 audit round); REDEM's
...
- [x] Figures 1–6 (vector `.pdf` in `../figures/`):
...
- [x] Tables: Table 1 (reward-only learners fail at the inversion; RLS
...
- [x] Code availability statement:
      ...
      acceptance).
- [x] Follow-up evidence integrated (2026-08-24): §4.4 causal audit re-run on
      ...
      plasticity correlation +0.57 mean on 7/10 seeds with paired t=2.04
      (95% CI includes 0) — audit resolution bounded); N=1024 at 10 seeds
      (s30 — full 0.9970 vs baseline 0.9753, paired t=15.3); §4.4
      novelty-gated rewiring (s25, 14.59 vs 12.43) and the M4-M5 coupling
      negative (s24).
- [x] Cover letter: the online / local / no-BPTT corner; self-regulation under
